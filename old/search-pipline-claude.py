...
class SearchKeywordGenerator:
    ...
    async def generate_search_terms(self, query: str, 
        ...
        4. Search operator combinations:
...
class SlackSearchSystem:
    def __init__(self):
        ...
        if not self.gemini_api_key or not self.slack_token:
        ...
        for query in search_terms:
            try:
                ...
                for msg in messages:
                    ...
                    text = self._clean_message(msg.get("text", ""))
...
async def main():
    try:
        ...
        start_date, end_date = date_extractor.extract_date_range(query)
        
        if start_date and end_date:
            print(f"検出された日付範囲: {start_date} から {end_date}")
        else:
            print("日付範囲は指定されていません")
        
        print("\n=== 検索処理を開始 ===")
        formatted_results = await search_system.process_query(
            query=query,
            channel_id=CHANNEL_ID,
            start_date=start_date,
            end_date=end_date,
            user_id=None  # ユーザーID機能は無効化
        )
